...
if(isset($_POST['sign']))
{
             ...
             $query="SELECT * FROM faculty_table WHERE fac_email='".$_POST['email']."'";
	$res=mysqli_query($conn,$query);
	$row=mysqli_fetch_array($res);

	
	if($_POST['pass']!=$row['fac_password'])
		{
			$error['email']="Username or password invalid";

		}else if($_POST['pass']==$row['fac_password']) {
			$email=$_POST['email'];
			$pass=$_POST['pass'];
			$_SESSION['fuser']=$row['fac_name'];
		header('Location: facultyPage.php');
	}
